let usrForm = document.getElementById("usr-form");
var UsrEntries = [];

let errors = [];
const retrieveEntries = () => {
  let entries = localStorage.getItem("UsrEntries");
  if (entries) {
...
const saveUserForm = (event) => {
  ...
  if (age < 18 || age > 55) {
    ...
    UsrEntries = retrieveEntries();
    UsrEntries.push(entry);
    localStorage.setItem("UsrEntries", JSON.stringify(UsrEntries));
    displayEntries();
    usrForm.reset();
  }
};
usrForm.addEventListener("submit", saveUserForm);
displayEntries();
